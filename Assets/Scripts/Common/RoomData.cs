...
{
    public static Dictionary<int, Data> roomData;
    public static Dictionary<int, ClearTime> clearTime;
    ...
    static RoomData()
    {
        roomData = new Dictionary<int, Data>()
        {
            ...
            { 4, new Data(new Vector2(0f, 4.5f),                                                            3, "THAT ONE WAS EASY.") },
            ...
            { 19, new Data(new Vector2(-7f, 0f),                                                            0, "NOT SO EASY, IS IT?" ) },
            { 20, new Data(new Vector2(9f, -3f),                                                            7, "IT GETS HARDER NOW." ) },
            { 21, new Data(new Vector2(-8f, 3f),                                                            3, "YOU'VE ALREADY LOST." ) },
            { 22, new Data(new Vector2(9f, 4f),                                                             0, "DON'T CHOKE!" ) },
            { 23, new Data(new Vector2(-4f, 4f),                                                            36, "AROUND AND AROUND..." ) },
            { 24, new Data(new Vector2(-9f, -4f),                                                           0, "THIS ONE ISN'T HARD IF YOU KNOW THE TRICK." ) },
            { 25, new Data(new Vector2(-8f, 4f),                                                            0, "YOU'RE PROBABLY GETTING FRUSTRATED." ) },
            { 26, new Data(new Vector2(0f, -3f),                                                            4, "SHOULDN'T EVEN TAKE MORE THAN 2 DEATHS..." ) },
            { 27, new Data(new Vector2(-9f, -2f),   new Vector2(9f, 0f),                                    1, "NOT HARD AT ALL." ) },
            { 28, new Data(new Vector2(-8f, -5f),   new Vector2(8f, 0f),                                    0, "BABY WANT HIS BOTTLE?" ) },
            { 29, new Data(new Vector2(8.5f, 4f),                                                           2, "MIGHT BE TRICKY." ) },
            { 30, new Data(new Vector2(-9f, -5f),                                                           3, "IMPOSSIBLE." ) }
        };

        clearTime = new Dictionary<int, ClearTime>();

        for (int i = 0; i <= 30; i++)
            clearTime[i] = new ClearTime();
    }
}